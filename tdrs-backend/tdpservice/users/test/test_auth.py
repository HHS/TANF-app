...
def resource(mocker):
    ...
    yield (mock_decode, mock_post, state, code)

class TestAuthentication:
    def test_func(self,resource):
        m_decode,y,_,_ = resource
        print("testing {}".format(m_decode.return_value))

        assert False
    
    @pytest.mark.django_db
    def test_login_with_valid_state_and_code(mocker, api_client, resource):
        """Test login with state and code."""
        m_decode,_,state,code = resource
        nonce = m_decode.return_value['nonce']
    ...
    @pytest.mark.django_db
    def test_login_with_existing_token(mocker, api_client, resource):
    ...
    @pytest.mark.django_db
    def test_login_with_general_exception(mocker, resource):
        """Test login with state and code."""
        m_decode,_,state,code = resource
        nonce = m_decode.return_value['nonce']
