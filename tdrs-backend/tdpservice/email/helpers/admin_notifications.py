...
from tdpservice.email.email_enums import AdminEmail
...
def email_admin_deactivated_user(user):
    ...
    template_path = AdminEmail.ACCOUNT_DEACTIVATED_ADMIN.value
    ...
    log(
        f"Preparing email to OFA Admins for deactivated user {user.username}",
...
def email_system_owner_system_admin_role_change(user, action):
    ...
    template_path = AdminEmail.SYSTEM_ADMIN_ROLE_CHANGED.value
    ...
    log(
        f"Preparing email to System Owner for System Admin role change for user {user.username}",
        logger_context=logger_context,
    )
