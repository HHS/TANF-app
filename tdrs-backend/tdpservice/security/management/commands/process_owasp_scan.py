...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--backend-pass-count", default=0, type=int)
        parser.add_argument("--backend-warn-count", default=0, type=int)
        parser.add_argument("--backend-fail-count", default=0, type=int)
        parser.add_argument("--frontend-pass-count", default=0, type=int)
        parser.add_argument("--frontend-warn-count", default=0, type=int)
        parser.add_argument("--frontend-fail-count", default=0, type=int)
        parser.add_argument("--project-slug", default="raft-tech/TANF-app", type=str)
    ...
    def handle(self, *args, **options):
        ...
        if response.status_code == 500:
            raise Exception(
                "CircleCI API returned 500 error. The 'circle_build_num' number may be incorrect."
            )
        elif response.status_code == 404:
            raise Exception(
                "CircleCI API returned 404 error. The 'circle_project_slug' number may be incorrect."
            )
        ...
        for artifact in artifacts:
            ...
            with TemporaryFile() as temp_file:
                ...
                OwaspZapScan.objects.record_scan(
                    app_target, html_report, fail_count, pass_count, warn_count
                )
