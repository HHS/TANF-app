...
def test_stts_is_valid_endpoint(api_client, stt_user_with_group):
    """Test an authorized user can successfully query the STT endpoint."""
    api_client.login(username=stt_user_with_group.username, password="test_password")
...
@pytest.mark.django_db
def test_stts_alpha_valid_endpoint(api_client, stt_user_with_group):
...
@pytest.mark.django_db
def test_stts_by_region_valid_endpoint(api_client, stt_user_with_group):
...
@pytest.mark.django_db
def test_can_get_stts(api_client, stt_user_with_group, stts):
...
@pytest.mark.django_db
def test_can_get_alpha_stts(api_client, stt_user_with_group, stts):
...
@pytest.mark.django_db
def test_alpha_stts_is_sorted(api_client, stt_user_with_group, stts):
...
@pytest.mark.django_db
def test_can_get_by_region_stts(api_client, stt_user_with_group, stts):
    """Test endpoint returns the alphabetized listing of STTs."""
    api_client.login(username=stt_user_with_group.username, password="test_password")
    response = api_client.get(reverse("stts-by-region"))
    assert response.status_code == status.HTTP_200_OK
...
@pytest.mark.django_db
def test_stts_and_stts_alpha_are_dissimilar(api_client, stt_user_with_group, stts):
