    ...
    def queryset(self, request, queryset):
        ...
        if self.value() is None and queryset.exists():
            ...
            for record in queryset.order_by(
                "datafile__stt__stt_code", "-datafile__id"
...
class STTFilter(MultiSelectDropdownFilter):
    ...
    def __init__(self, field, request, params, model, model_admin, field_path):
        super(MultiSelectDropdownFilter, self).__init__(
    ...
    def lookups(self, request, model_admin):
        ...
        listing_record_type = [
            i for i in str(request.path).lower().split("/") if i not in ["/", ""]
        ][-1]
        options = []
...
class FiscalPeriodFilter(SimpleListFilter):
    ...
    def queryset(self, request, queryset):
        ...
        if self.value() is not None and queryset.exists():
            ...
            queryset = queryset.filter(datafile__quarter=quarter).filter(
                datafile__year=year
            )
