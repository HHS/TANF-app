...
from tdpservice.parsers import schema_defs, util
...
class TestHeaderSchema:
    """Tests for header schema parsing."""
    ...
    def test_header_cleanup(self, test_datafile):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # quarter error
            (
                "HEADER20205A06   TAN1ED",
                False,
                "HEADER Item 5 (quarter): 5 is not in [1, 2, 3, 4].",
            ),
            # Type error
            (
                "HEADER20204E06   TAN1ED",
                False,
                "HEADER Item 6 (type): E is not in [A, C, G, S].",
            ),
            # Fips error
            (
                "HEADER20204A07   TAN1ED",
                True,
                (
                    "HEADER Item 1 (state fips): 07 is not in [00, 01, 02, 04, 05, 06, 08, 09, "
            ...
            # Tribe error
            (
                "HEADER20204A00 -1TAN1ED",
                True,
                "HEADER Item 3 (tribe code):  -1 is not in range [0, 999].",
            ),
            # Program type error
            (
                "HEADER20204A06   BAD1ED",
                False,
                "HEADER Item 7 (program type): BAD is not in [TAN, SSP].",
            ),
            # Edit error
            ("HEADER20204A06   TAN3ED", False, "HEADER Item 8 (edit): 3 is not in [1, 2]."),
            # Encryption error
            (
                "HEADER20204A06   TAN1AD",
                True,
                "HEADER Item 9 (encryption): A is not in [ , E].",
            ),
            # Update error
            (
                "HEADER20204A06   TAN1EA",
                True,
                (
                    "HEADER Update Indicator must be set to D instead of A. Please review "
    ...
    @pytest.mark.django_db
    def test_header_fields(self, test_datafile, header_line, is_valid, error):
