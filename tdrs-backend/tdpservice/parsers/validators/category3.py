...
def isOlderThan(min_age):
    ...
    return make_validator(
        _validate,
        lambda eargs: f"{str(eargs.value)[:4]} must be less "
...
def validateSSN():
    ...
    return make_validator(base.isNotOneOf(options), lambda eargs: f"is in {options}.")
...
def orValidators(validators, **kwargs):
    ...
    def _validate(value, eargs):
        ...
        if not any(result.valid for result in validator_results):
            error_msg = (
                f"{format_error_context(eargs)} {value} "
                if not is_if_result_func
                else ""
            )
            error_msg += (
...
def suppress_for_fra_pilot_state(condition_field_name, result_field_name, validator):
    ...
    def validate(record, row_schema):
        ...
        if (
            stt.type is not None
            and stt.type.lower() == "state"
            and stt.postal_code in pilotStates
        ):
...
def validate__WORK_ELIGIBLE_INDICATOR__HOH__AGE():
    ...
    def validate(record, row_schema):
        ...
        try:
            WORK_ELIGIBLE_INDICATOR = get_record_value_by_field_name(
                record, "WORK_ELIGIBLE_INDICATOR"
            )
            RELATIONSHIP_HOH = int(
                get_record_value_by_field_name(record, "RELATIONSHIP_HOH")
            )
            ...
            RPT_MONTH_YEAR_datetime = datetime.datetime.strptime(
                RPT_MONTH_YEAR, "%Y%m%d"
            )
