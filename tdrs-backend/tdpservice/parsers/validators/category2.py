...
def dateYearMonthIsLargerThan(year, month, **kwargs):
    ...
    def _validator(value: str):
        ...
        value_month = int(value[4:])

        _year_validator = base.isGreaterThan(year, **kwargs)
        _month_validator = base.isGreaterThan(month, **kwargs)

        if value_year == year:
            return make_validator(
                lambda _: _month_validator(value_month),
                lambda eargs: f"{format_error_context(eargs)} If year equals {year} month must be larger than {month}.",
            )
