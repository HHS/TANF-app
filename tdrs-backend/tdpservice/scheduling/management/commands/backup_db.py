...
from tdpservice.scheduling.management.db_backup import get_system_values, main
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not settings.USE_LOCALSTACK:
            ...
            export_password = (
                f"export PGPASSWORD={settings.DATABASES['default']['PASSWORD']}"
            )
            try:
                cmd = (
                    f"{export_password} && pg_dump -h {db_host} -p {db_port} -d {db_name} -U {db_user} -F c "
