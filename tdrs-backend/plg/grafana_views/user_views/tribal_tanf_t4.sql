...
-- SQL view for tribal_tanf_t4 schema
-- Transformations applied:


CREATE OR REPLACE VIEW "tribal_tanf_t4" AS SELECT "RecordType","RPT_MONTH_YEAR","CASE_NUMBER","STRATUM","ZIP_CODE","DISPOSITION","CLOSURE_REASON","REC_SUB_HOUSING","REC_MED_ASSIST","REC_FOOD_STAMPS","REC_SUB_CC", 
    data_files.section,
    data_files.version,
    data_files.year,
    data_files.quarter,
...
FROM search_indexes_TRIBAL_TANF_T4 T4
INNER JOIN
        data_files_datafile data_files                                             -- Join with data_files_datafile
        ON T4.datafile_id = data_files.id                              -- Join condition
    INNER JOIN (
        SELECT
