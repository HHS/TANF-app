...
-- SQL view for ssp_m6 schema
-- Transformations applied:


CREATE OR REPLACE VIEW "ssp_m6" AS SELECT "RecordType","CALENDAR_QUARTER","RPT_MONTH_YEAR","SSPMOE_FAMILIES","NUM_2_PARENTS","NUM_1_PARENTS","NUM_NO_PARENTS","NUM_RECIPIENTS","ADULT_RECIPIENTS","CHILD_RECIPIENTS","NONCUSTODIALS","AMT_ASSISTANCE","CLOSED_CASES", 
    data_files.section,
    data_files.version,
    data_files.year,
    data_files.quarter,
...
FROM search_indexes_SSP_M6 M6
INNER JOIN
        data_files_datafile data_files                                             -- Join with data_files_datafile
        ON M6.datafile_id = data_files.id                              -- Join condition
    INNER JOIN (
        SELECT
