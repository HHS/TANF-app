...
-- SQL query for tribal_tanf_t5 schema
-- Transformations applied:
--   * DATE_OF_BIRTH transformed to AGE calculation (as integer)
--   * SSN transformed to md5 hash for privacy


SELECT "RecordType","RPT_MONTH_YEAR","CASE_NUMBER","FAMILY_AFFILIATION",-- Calculate AGE_FIRST: Age as of the first day of the reporting month
                CASE 
                ...
                CASE 
                    ...
                    ELSE NULL 
                END AS "AGE_LAST",md5("SSN"::text) as "SSN_HASH","RACE_HISPANIC","RACE_AMER_INDIAN","RACE_ASIAN","RACE_BLACK","RACE_HAWAIIAN","RACE_WHITE","SEX","REC_OASDI_INSURANCE","REC_FEDERAL_DISABILITY","REC_AID_TOTALLY_DISABLED","REC_AID_AGED_BLIND","REC_SSI","MARITAL_STATUS","RELATIONSHIP_HOH","PARENT_MINOR_CHILD","NEEDS_OF_PREGNANT_WOMAN","EDUCATION_LEVEL","CITIZENSHIP_STATUS","COUNTABLE_MONTH_FED_TIME","COUNTABLE_MONTHS_STATE_TRIBE","EMPLOYMENT_STATUS","AMOUNT_EARNED_INCOME","AMOUNT_UNEARNED_INCOME", 
    data_files.section,
    data_files.version,
    data_files.year,
    data_files.quarter,
...
FROM search_indexes_TRIBAL_TANF_T5 T5
INNER JOIN
        data_files_datafile data_files                                             -- Join with data_files_datafile
        ON T5.datafile_id = data_files.id                              -- Join condition
    INNER JOIN (
        SELECT
