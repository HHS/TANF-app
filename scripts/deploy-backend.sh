...
update_backend()
{
    ...
    if [ "$1" = "rolling" ] ; then
        ...
        cf push "$CGHOSTNAME_BACKEND" --no-route -b https://github.com/cloudfoundry/apt-buildpack -b manifest.buildpack.yml  --strategy rolling || exit 1
    else
        cf push "$CGHOSTNAME_BACKEND" --no-route -b https://github.com/cloudfoundry/apt-buildpack -b manifest.buildpack.yml
