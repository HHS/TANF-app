...
update_backend()
{
    ...
    if [ "$1" = "rolling" ] ; then
        ...
        cf push "$CGHOSTNAME_BACKEND" --no-route  -f manifest.buildpack.yml -b https://github.com/cloudfoundry/apt-buildpack  --strategy rolling || exit 1
    else
        cf push "$CGHOSTNAME_BACKEND" --no-route  -f manifest.buildpack.yml -b https://github.com/cloudfoundry/apt-buildpack
