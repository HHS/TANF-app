...
update_backend()
{
    ...
    if [ "$1" = "rolling" ] ; then
        ...
        cf push "$CGAPPNAME_BACKEND" --no-route -f manifest.buildpack.yml -t HEALTH-CHECK-TIMEOUT --strategy rolling || exit 1
    else
        cf push "$CGAPPNAME_BACKEND" --no-route -f manifest.buildpack.yml -t HEALTH-CHECK-TIMEOUT
