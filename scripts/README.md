...
+ [configure-admin-production-env.sh](./README.md#configure-admin-production-envsh)
...
## [configure-admin-production-env.sh](./configure-admin-production-env.sh)

Configures the standalone admin console, Django backend, and Keycloak
environment variables in the production Cloud Foundry space. It is intended
for initial production provisioning and coordinated secret rotation. It does
not support dev or staging.

The script is dry-run by default, verifies the exact production organization,
space, and application names before applying, redacts secrets, and refuses to
replace existing secrets unless `--rotate-secrets` is explicitly supplied.
It does not restart applications.

### Production procedure

1. Target production and identify the admin application name:
   ...
   Confirm that `https://admin.tanfdata.acf.hhs.gov` is mapped to the selected
   admin app. The internal backend URL configured by the script also requires
   this network policy:
...
2. Generate two independent 32-byte secrets and save both values in the
   approved secret manager immediately:
   ...
   `ADMIN_API_PROXY_TOKEN` is installed on the admin and Django applications.
   `KC_TDP_ADMIN_CLIENT_SECRET` is installed on Keycloak and copied to Django
   as `KEYCLOAK_TDP_ADMIN_CLIENT_SECRET`. The Admin OIDC client secret is not
   automatically generated by Keycloak or Django.

   If the backend is bound to the `tdp-keycloak-prod` user-provided service and
   that service already supplies `tdp_admin_client_id` or
   `tdp_admin_client_secret`, Django gives those credentials precedence over
   application environment variables. The script detects conflicting bound
   credentials and stops with instructions instead of applying an ineffective
   value.

3. Run a dry run and review every target and non-secret value:
...
4. Apply the environment variables after approval:
   ...
   For a planned rotation of existing secrets, add `--rotate-secrets`. This
   flag is not needed for initial provisioning or when supplied values already
   match Cloud Foundry.

5. Activate the values in this order during the deployment window:

   ```bash
   cf restage keycloak
   cf logs keycloak --recent
   # Confirm: Keycloak config import complete.

   cf restage tdp-backend-prod
   cf restage <production-admin-app>
   ```

6. Verify Login.gov and AMS admin login, logout, session isolation, and one
   read plus one CSRF-protected write through `/api/admin/*`.

The complete procedure and safety notes are also available from:

```bash
./scripts/configure-admin-production-env.sh --help
```
...
```bash
./scripts/deploy-backend.sh <strategy> <frontend-hostname> <backend-app-name> <cf-space> [all|backend|celery]
./scripts/deploy-backend.sh rolling raft tdp-backend-raft tanf-dev backend
```
### Arguments
...
### Description
Mostly for use in our CircleCI pipelines, this script deploys the Django backend or Python Celery worker into a Cloud.gov application space. The optional component argument allows CircleCI to run those deployments in parallel; omitting it retains the combined manual recovery behavior.

### Where it's used

`deploy-backend.sh` is used by the independent backend and Celery CircleCI jobs. Shared migrations run before both jobs, and networking is configured after all application deployments succeed. With Cloud Foundry set up and logged in locally, the script can also be used manually. See the [CircleCI documentation](../docs/Technical-Documentation/circle-ci.md#deploy-backend).
...
`deploy-frontend.sh` is used by the independent frontend CircleCI job. With Cloud Foundry set up and logged in locally, it can also be used manually. See the [CircleCI documentation](../docs/Technical-Documentation/circle-ci.md#deploy-frontend).
